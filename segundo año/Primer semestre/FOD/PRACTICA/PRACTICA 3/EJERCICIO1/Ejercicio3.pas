...
procedure leer(var arch:archivo_logico;var e:empleado);
begin
...
procedure darBajaPersona(var arch:archivo_logico);
var
	pos:integer;
	e:empleado;
	numEmpleado:integer;
...
begin
	...
	writeln ('Ingrese num de empleado');
	readln (numEmpleado);
	reset (arch);
	leer(arch,e);
	while (e.num <> numEmpleado)do  //el empleado puede no estar
		begin
			leer(arch,e);
		end;	
	if (e.num <> valoralto)then
		begin
			pos := FilePos(arch)-1;  //guardo la pos 
			Seek(arch,FileSize(arch)-1);	//ultimo arch
			leer(arch,e);	//leo lo que esta en el ultimo
			Seek(arch,pos);	//me posiciono en el que tengo q borrar
			write(arch,e);	//escribo el ultimo registro
			Seek(arch,FileSize(arch)-1);	//me posiciono
...
BEGIN
	...
	while (ok) do
		begin
			...
			writeln('9 PARA DAR DE BAJA A UN EMPLEADO');
			writeln('10 PARA SALIR');
			readln(n);
			writeln;
			writeln;
			case n of
