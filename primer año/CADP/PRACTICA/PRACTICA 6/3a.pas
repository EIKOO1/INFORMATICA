...
nuevo:lista;
begin
new(nuevo);//genero un nuevo espacio en memoria dinamica
nuevo^.num := v;//nuevo le asigno el valor que ingreso por teclado
nuevo^.sig := nil;//la direccion a la que apunta es nil
if (l = nil) then //si es el primer elemento 
l:= nuevo//la direcion es la de nuevo
else

begin
aux:= l;//posiciono a aux en la primera posicion de la lista
while (aux^.sig <> nil ) do//mientras a lo que apunta aux sea distinto a nil
aux:= aux^.sig;//aux recorre todo la lista
aux^.sig:= nuevo;//al salir del while es 1porque aux^.sig es = nil entonces a esa direccion le asigno el nuevo elemento
end;
end;
